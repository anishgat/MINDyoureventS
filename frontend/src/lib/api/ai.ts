...
export async function generateEventBackground(prompt: string) {
  if (!prompt.trim()) {
  ...
  const response = await ai.models.generateContent({
    ...
    contents: prompt,
    config: {
      systemInstruction: `Generate an image of a group of friends participating in the activity in the prompt
      `
    }
  });
...
export async function generateIcon(prompt: string) {
  ...
  const response = await ai.models.generateContent({
    ...
    config: {
      ...
      systemInstruction: `Your job is to generate an animated cartoon-style sticker of the activity in the prompt. If a location is specified in the prompt, add an animated background of the location where appropriate. Use a vibrant color palette. 
      `
    }
  });
